    ...
    return (
      <div className="min-h-screen bg-felt flex items-center justify-center">
        <div className="flex flex-col items-center gap-4" role="status" aria-label="Loading scorecard">
          ...
          <span className="font-sans text-cream-dim uppercase tracking-[0.3em] text-sm" style={{ opacity: 0.4 }} aria-hidden="true">Loading</span>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-felt flex flex-col">
      ...
      <div className="mx-auto w-full max-w-lg flex items-center gap-3 px-4 py-3">
        ...
        <div className="flex-1 flex items-center justify-center gap-2">
          <span className="font-sans text-cream-dim uppercase tracking-[0.3em] text-sm" style={{ opacity: 0.7 }}>Game</span>
          <span
          ...
          <span
            className="font-sans uppercase tracking-[0.2em] text-sm"
            style={{
        ...
        >
          ...
          <thead>
            <tr className="bg-felt-deep" style={{ borderBottom: '1px solid oklch(72% 0.13 82 / 15%)' }}>
              <th className="font-sans text-cream-dim uppercase text-center px-2 py-3 text-sm" style={{ letterSpacing: '0.25em', opacity: 0.6, width: '3rem' }}>Rnd</th>
              <th className="font-sans text-cream-dim uppercase text-center px-2 py-3 text-sm" style={{ letterSpacing: '0.25em', opacity: 0.6, width: '2.5rem' }}>Trump</th>
              {players.map((name, i) => (
                <th key={i} className="font-sans text-gold uppercase text-center px-2 py-3 text-sm" style={{ letterSpacing: '0.15em', opacity: 0.8 }}>
          ...
          <tbody>
              ...
              return (
                ...
                >
                  ...
                  <td className="text-center px-2 py-2.5">
                    <span className="font-serif text-gold text-sm" style={{ opacity: 0.7 }}>{round.roundNumber}</span>
                  </td>

                  {/* Trump suit */}
                  <td className="text-center px-2 py-2.5">
                    {round.roundNumber === 1 || !round.trumpSuit ? (
                      <span className="font-sans text-cream-dim text-sm" style={{ opacity: 0.25 }}>—</span>
                    ) : (
                      <span
                        ...
                        {res ? (
                          <div className="flex flex-col items-center gap-0.5">
                            <div className="flex items-center gap-0.5">
                              ...
                              {perfect && <span className="text-sm" style={{ lineHeight: 1 }}>🔥</span>}
                            </div>
                            <span className="font-sans text-sm" style={{ color: 'oklch(72% 0.13 82)', opacity: 0.55 }}>
                              +{res.points}
                            </span>
                            <span className="font-serif text-cream text-sm" style={{ opacity: 0.55 }}>
                              {cum[i]}
                            </span>
                          </div>
                        ) : (
                          <span className="font-sans text-cream-dim text-sm" style={{ opacity: 0.2 }}>—</span>
                        )}
                      </td>
                    )
                  })}
            ...
            {inProgressRound && (
              <tr className="bg-felt-inset" style={{ borderBottom: '1px solid oklch(72% 0.13 82 / 8%)' }}>
                {/* Round number */}
                <td className="text-center px-2 py-2.5">
                  <span className="font-serif text-gold text-sm" style={{ opacity: 0.55 }}>{inProgressRound.roundNumber}</span>
                </td>
                  ...
                  return (
                    <td key={i} className="text-center px-2 py-2" style={{ verticalAlign: 'middle' }}>
                      <div className="flex flex-col items-center gap-0.5">
                        ...
                        {/* "in progress" hint */}
                        <span className="font-sans text-sm" style={{ color: 'oklch(72% 0.13 82)', opacity: 0.3, letterSpacing: '0.1em' }}>
          ...
          <tfoot>
            <tr className="bg-felt-deeper" style={{ borderTop: '1px solid oklch(72% 0.13 82 / 20%)' }}>
              <td colSpan={2} className="px-2 py-3">
                <span className="font-sans text-cream-dim uppercase tracking-[0.25em] text-sm" style={{ opacity: 0.55 }}>Total</span>
              </td>
              {players.map((_, i) => {
